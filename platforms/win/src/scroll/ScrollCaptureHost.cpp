...
constexpr wchar_t scrollCaptureWindowClass[] =
    L"XxSnapScrollCaptureChromeWindow";
...
struct IconBitmap final {
    ...
    IconBitmap() = default;
    ~IconBitmap() { if (bitmap != nullptr) DeleteObject(bitmap); }
...
int scaledDip(float dip, UINT dpi) noexcept
{
    return static_cast<int>(std::lround(
        dip * static_cast<float>(dpi == 0U ? 96U : dpi) / 96.0F));
}
...
std::optional<PixelRect> detectTargetWithTimeout(
    PixelRect selection,
    ...
    int targetEdge) noexcept
{
    ...
    const auto resource = FindResourceW(
        instance, MAKEINTRESOURCEW(resourceId), MAKEINTRESOURCEW(10));
    ...
    const auto bitmap = CreateDIBSection(
        screen, &bitmapInfo, DIB_RGB_COLORS, &dibBits, nullptr, 0U);
...
struct ScrollCaptureHost::Impl final {
    ...
    Impl(std::size_t maximumBytes)
        : session(maximumBytes)
        , capturer(maximumBytes)
    {
    }
    ...
    LRESULT handleMessage(HWND window, UINT message, WPARAM wParam, LPARAM lParam) noexcept
    {
        ...
        case WM_LBUTTONUP:
            if (window == toolbarWindow) {
                ...
                if (!reviewing && PtInRect(&finishButton, point)) {
                    finish();
                } else if (reviewing && PtInRect(&cancelButton, point)) {
            ...
            return 0;
        case WM_MOUSEWHEEL:
            if (window == previewWindow && reviewing) {
                reviewOffset -= static_cast<std::int64_t>(
        ...
        case scrollWheelMessage:
            ...
            SetTimer(toolbarWindow, captureTimerIdentifier,
                captureSettleMilliseconds, nullptr);
            return 0;
        case WM_TIMER:
    ...
    bool initialize() noexcept
    {
        ...
        mouseHook = SetWindowsHookExW(
            WH_MOUSE_LL, &Impl::hookMouse, instance, 0U);
        keyboardHook = SetWindowsHookExW(
            WH_KEYBOARD_LL, &Impl::hookKeyboard, instance, 0U);
    ...
    bool createWindows() noexcept
    {
        ...
        const auto toolbarWidth = static_cast<std::int64_t>(std::lround(
            (toolbarWidthDip() + ToolbarMetrics::buttonStepDip) * scaleX));
        const auto toolbarHeight = scaledDip(ToolbarMetrics::heightDip, dpiY);
        auto toolbarX = selection.x + (selection.width - toolbarWidth) / 2;
        toolbarX = (std::max)(workArea.x,
            (std::min)(toolbarX, workArea.x + workArea.width - toolbarWidth));
        ...
        toolbarY = (std::max)(workArea.y,
            (std::min)(toolbarY, workArea.y + workArea.height - toolbarHeight));
        ...
        previewX = (std::max)(workArea.x,
            (std::min)(previewX, workArea.x + workArea.width - previewWidth));
        auto previewY = selection.y + selection.height - previewHeight;
        previewY = (std::max)(workArea.y,
            (std::min)(previewY, workArea.y + workArea.height - previewHeight));
        ...
        const auto outer = CreateRectRgn(
            0, 0, static_cast<int>(selection.width),
            static_cast<int>(selection.height));
        const auto inner = CreateRectRgn(
            border, border,
            static_cast<int>(selection.width) - border,
        ...
        SetWindowRgn(toolbarWindow,
            CreateRoundRectRgn(0, 0, static_cast<int>(toolbarBounds.width) + 1,
                static_cast<int>(toolbarBounds.height) + 1, radius, radius), FALSE);
        computeFinishButton();
        return true;
    }
    ...
    HWND createWindow(DWORD extendedStyle, PixelRect bounds) noexcept
    {
        return CreateWindowExW(
            extendedStyle, scrollCaptureWindowClass, L"", WS_POPUP,
    ...
    {
        return toolbarWidth(std::vector<ToolbarAction>{
    ...
    void finish() noexcept
    {
        ...
        if (borderWindow != nullptr) ShowWindow(borderWindow, SW_HIDE);
        const auto previewStyle = GetWindowLongPtrW(previewWindow, GWL_EXSTYLE);
        SetWindowLongPtrW(
            previewWindow, GWL_EXSTYLE,
            previewStyle & ~static_cast<LONG_PTR>(WS_EX_TRANSPARENT));
        const auto editorWidth = (std::min)(
            workArea.width - scaledDip(40.0F, dpiX),
            (std::max)(static_cast<std::int64_t>(scaledDip(520.0F, dpiX)),
                workArea.width * 3 / 4));
        const auto editorHeight = (std::min)(
            workArea.height - scaledDip(100.0F, dpiY),
            (std::max)(static_cast<std::int64_t>(scaledDip(420.0F, dpiY)),
                workArea.height * 3 / 4));
        previewBounds = {
            workArea.x + (workArea.width - editorWidth) / 2,
            workArea.y + (workArea.height - editorHeight) / 2,
            editorWidth,
            editorHeight,
        };
        SetWindowPos(previewWindow, HWND_TOPMOST,
            static_cast<int>(previewBounds.x), static_cast<int>(previewBounds.y),
            static_cast<int>(previewBounds.width),
            static_cast<int>(previewBounds.height),
            SWP_NOACTIVATE | SWP_SHOWWINDOW);
        const auto toolbarY = (std::min)(
            workArea.y + workArea.height - toolbarBounds.height,
            previewBounds.y + previewBounds.height + scaledDip(8.0F, dpiY));
        toolbarBounds.y = toolbarY;
        SetWindowPos(toolbarWindow, HWND_TOPMOST,
            static_cast<int>(toolbarBounds.x), static_cast<int>(toolbarBounds.y),
            static_cast<int>(toolbarBounds.width),
            static_cast<int>(toolbarBounds.height),
            SWP_NOACTIVATE | SWP_SHOWWINDOW);
        InvalidateRect(previewWindow, nullptr, FALSE);
    ...
    void clampReviewOffset() noexcept
    {
        if (!finalPixels.has_value()) {
            reviewOffset = 0;
            return;
        }
        const auto viewportWidth = (std::max)(
            1LL, previewBounds.width - scaledDip(24.0F, dpiX));
        const auto viewportHeight = (std::max)(
            1LL, previewBounds.height - scaledDip(50.0F, dpiY));
        const auto scale = (std::min)(1.0,
            static_cast<double>(viewportWidth) / finalPixels->width());
        const auto visibleSourceHeight = static_cast<std::int64_t>(
            viewportHeight / scale);
        const auto maximum = (std::max)(
            0LL, finalPixels->height() - visibleSourceHeight);
        reviewOffset = (std::max)(0LL, (std::min)(reviewOffset, maximum));
    }
    ...
    void paint(HWND window) noexcept
    {
        ...
        if (window == borderWindow) {
            const auto color = targetResolved
                ? RGB(52, 199, 89)
                : RGB(83, 120, 232);
        ...
        BYTE alpha) noexcept
    {
        ...
        AlphaBlend(target, destination.left, destination.top,
            ...
            source, 0, 0, icon.width, icon.height, blend);
    ...
    void paintToolbar(HDC dc, RECT client) noexcept
    {
        const auto background = CreateSolidBrush(RGB(
            VisualStyleCatalog::toolbarBackgroundColor.red,
    ...
    void paintPreview(HDC dc, RECT client) noexcept
    {
        ...
        FillRect(dc, &client, background);
        DeleteObject(background);
        if (reviewing && finalPixels.has_value()) {
            paintLongImage(dc, client);
            return;
        }
        ...
        const auto viewportHeight = (std::max)(2,
        ...
        const auto viewportY = direction == snipory::core::scroll::ScrollDirection::Up
            ? y : y + height - (std::min)(height, viewportHeight);
        const auto blue = CreatePen(PS_SOLID,
            (std::max)(1, scaledDip(2.0F, dpiX)), RGB(0, 122, 255));
        ...
        const auto font = CreateFontW(
            -scaledDip(12.0F, dpiY), 0, 0, 0, FW_MEDIUM, FALSE, FALSE,
        ...
        DrawTextW(dc, label.data(), static_cast<int>(label.size()),
            &labelRect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
    ...
    void paintCaptureNotice(HDC dc, RECT client) noexcept
    {
        ...
        const auto region = CreateRoundRectRgn(
            panel.left, panel.top, panel.right, panel.bottom, radius, radius);
        ...
        RECT accent{
            ...
            panel.left + scaledDip(4.0F, dpiX), panel.bottom,
        };
        ...
        const auto oldFont = SelectObject(dc, font);
        RECT textRect{
            panel.left + scaledDip(14.0F, dpiX), panel.top,
            panel.right - scaledDip(8.0F, dpiX), panel.bottom,
        };
        DrawTextW(dc, captureNotice.data(),
            static_cast<int>(captureNotice.size()), &textRect,
            DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS);
        SelectObject(dc, oldFont);
        DeleteObject(font);
    }

    void paintLongImage(HDC dc, RECT client) noexcept
    {
        const auto& pixels = *finalPixels;
        const auto margin = scaledDip(12.0F, dpiX);
        const auto labelHeight = scaledDip(26.0F, dpiY);
        const auto viewportWidth = (std::max)(1,
            static_cast<int>(client.right - client.left) - margin * 2);
        const auto viewportHeight = (std::max)(
            1, static_cast<int>(client.bottom - client.top)
                - margin * 2 - labelHeight);
        const auto scale = (std::min)(1.0,
            static_cast<double>(viewportWidth) / pixels.width());
        const auto sourceHeight = (std::min)(
            pixels.height(),
            static_cast<std::int64_t>(viewportHeight / scale));
        clampReviewOffset();
        const auto destinationHeight = (std::max)(
            1, static_cast<int>(sourceHeight * scale));
        const auto destinationWidth = (std::max)(
            1, static_cast<int>(pixels.width() * scale));
        const auto x = (client.right - destinationWidth) / 2;
        const auto y = margin;
        BITMAPINFO info{};
        info.bmiHeader.biSize = sizeof(BITMAPINFOHEADER);
        ...
        info.bmiHeader.biCompression = BI_RGB;
        SetStretchBltMode(dc, HALFTONE);
        StretchDIBits(dc, x, y, destinationWidth, destinationHeight,
            0, static_cast<int>(reviewOffset),
            static_cast<int>(pixels.width()), static_cast<int>(sourceHeight),
        ...
        const auto font = CreateFontW(
            ...
            CLEARTYPE_QUALITY, DEFAULT_PITCH, L"Microsoft YaHei");
        const auto oldFont = SelectObject(dc, font);
        const auto label = std::to_wstring(pixels.width()) + L" x "
            + std::to_wstring(pixels.height()) + L" px";
        RECT labelRect{0, client.bottom - labelHeight, client.right, client.bottom};
        DrawTextW(dc, label.data(), static_cast<int>(label.size()),
            &labelRect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
        SelectObject(dc, oldFont);
        DeleteObject(font);
