...
final class SelectionOverlayWindow: NSWindow {
    ...
    var test_undoActionCount: Int {
        (contentView as? SelectionOverlayView)?.test_undoActionCount ?? 0
    }
...
private final class SelectionOverlayView: NSView, NSTextViewDelegate {
    ...
    private var undoActions: [CaptureHistoryAction] = []
    private var redoActions: [CaptureHistoryAction] = []
    private var nextRenderOrderValue = 1
    ...
    override func mouseUp(with event: NSEvent) {
        ...
        case .drawingShape:
            ...
            if let draft = draftAnnotation, isUsableDraftAnnotation(draft) {
                let orderedDraft = assignRenderOrder(to: draft)
                annotations.append(orderedDraft)
                recordUndo(.addedAnnotation(orderedDraft))
                selectedAnnotationIndex = orderedDraft.kind == .brush || orderedDraft.kind == .mosaicStroke ? nil : annotations.indices.last
                currentStyle = orderedDraft.style
                if orderedDraft.kind == .magnifier {
                    magnifierStyle = orderedDraft.style
                } else {
                    currentShapeKind = orderedDraft.kind
                    activeShapeKind = orderedDraft.kind
                }
                rememberCurrentStyleForActiveTool()
                if orderedDraft.kind == .mosaicStroke || orderedDraft.kind == .mosaicRectangle {
                    resetMosaicRedactionPreviewCaches()
                }
                NSLog("xxsnap overlay added annotation count=%ld rect=(%.0f, %.0f, %.0f, %.0f)", annotations.count, orderedDraft.rect.minX, orderedDraft.rect.minY, orderedDraft.rect.width, orderedDraft.rect.height)
            }
            shapeStartPoint = nil
    ...
    private func handleAnnotatingMouseDown(at point: NSPoint, clickCount: Int = 1) {
        ...
        if isEraserToolActive {
            beginEraserInteraction(at: point)
            return
        }
    ...
    private func createNumberMark(at point: NSPoint) {
        ...
        let orderedAnnotation = assignRenderOrder(to: annotation)
        annotations.append(orderedAnnotation)
        recordUndo(.addedAnnotation(orderedAnnotation))
    ...
    private func assignRenderOrder(to annotation: CaptureAnnotation) -> CaptureAnnotation {
        var copy = annotation
        if copy.renderOrder == 0 {
            copy.renderOrder = nextRenderOrder()
        }
        return copy
    }

    private func nextRenderOrder() -> Int {
        let value = nextRenderOrderValue
        nextRenderOrderValue += 1
        return value
    }

    private func recordUndo(_ action: CaptureHistoryAction) {
        undoActions.append(action)
        redoActions.removeAll()
    }

    private func commitTextDraftCreationToHistoryIfNeeded(at index: Int) {
        guard textDraftCreatedDuringCurrentEdit, annotations.indices.contains(index) else {
            return
        }
        let orderedAnnotation = assignRenderOrder(to: annotations[index])
        annotations[index] = orderedAnnotation
        recordUndo(.addedAnnotation(orderedAnnotation))
    }
    ...
    private func prepareEditingTextForForwardedDrag(at index: Int) -> Bool {
        ...
        selectedAnnotationIndex = index
        editingTextAnnotationIndex = nil
        commitTextDraftCreationToHistoryIfNeeded(at: index)
        textDraftCreatedDuringCurrentEdit = false
    ...
    private func commitCurrentTextEdit(restoreWindowLevel: Bool = true) {
        ...
        commitTextDraftCreationToHistoryIfNeeded(at: editingTextAnnotationIndex)
        self.editingTextAnnotationIndex = nil
        textDraftCreatedDuringCurrentEdit = false
        clearPendingTextEdit()
        removeTextEditor(restoreWindowLevel: restoreWindowLevel)
    ...
    private func finishEraserClick(at point: NSPoint) {
        guard let hitIndex = topmostAnnotationIndex(at: point) else {
            cancelEraserDraft()
            return
        }
    ...
    func test_setAnnotations(_ newAnnotations: [CaptureAnnotation]) {
        ...
        nextRenderOrderValue = (newAnnotations.map(\.renderOrder).max() ?? 0) + 1
    ...
    private var selectedAnnotation: CaptureAnnotation? {
        ...
        guard !isAnnotationErasedForInteraction(at: selectedAnnotationIndex) else {
            return nil
        }
    ...
    private func selectAnnotation(at index: Int) {
        ...
        guard !isAnnotationErasedForInteraction(at: index) else {
            selectedAnnotationIndex = nil
            return
        }
    ...
    private func topmostAnnotationIndex(at point: NSPoint) -> Int? {
        for index in annotations.indices.reversed() {
            guard !isAnnotationErasedForInteraction(at: index) else {
                continue
            }
            if annotationHitTest(point: point, annotation: annotations[index], index: index) {
                return index
            }
        }
        return nil
    ...
    private func annotationHitTest(point: NSPoint, annotation: CaptureAnnotation, index: Int) -> Bool {
        guard annotations.indices.contains(index) else {
            return false
        }
        if annotationBorderContains(point, for: annotation) {
            return true
        }
    ...
    private func isAnnotationErasedForInteraction(at index: Int) -> Bool {
        guard annotations.indices.contains(index), !eraserMasks.isEmpty else {
            return false
        }
        let annotation = annotations[index]
        let annotationOrder = annotation.renderOrder == 0 ? index + 1 : annotation.renderOrder
        let bounds = localInteractionBounds(for: annotation).standardized
        guard !bounds.isEmpty else {
            return false
        }

        return eraserMasks.contains { mask in
            let maskOrder = mask.renderOrder == 0 ? Int.max : mask.renderOrder
            guard maskOrder > annotationOrder else {
                return false
            }
            guard let maskBounds = localInteractionBounds(for: mask)?.standardized, !maskBounds.isEmpty else {
                return false
            }
            return bounds.intersects(maskBounds)
        }
    }

    private func localInteractionBounds(for annotation: CaptureAnnotation) -> NSRect {
        let strokeOutset = max(4, annotation.style.strokeWidth / 2 + 4)
        switch annotation.kind {
        case .arrowLine:
            guard let arrowLine = annotation.arrowLine else {
                return annotation.rect.standardized.insetBy(dx: -strokeOutset, dy: -strokeOutset)
            }
            return localBounds(of: [arrowLine.start, arrowLine.end, arrowLine.control])
                .insetBy(dx: -strokeOutset, dy: -strokeOutset)
        case .brush:
            return localBounds(of: annotation.brushPath?.points ?? [])
                .insetBy(dx: -strokeOutset, dy: -strokeOutset)
        case .marker:
            guard let markerLine = annotation.markerLine else {
                return annotation.rect.standardized.insetBy(dx: -strokeOutset, dy: -strokeOutset)
            }
            return localBounds(of: [markerLine.start, markerLine.end])
                .insetBy(dx: -strokeOutset, dy: -strokeOutset)
        case .mosaicStroke:
            return localBounds(of: annotation.mosaicStroke?.points ?? [])
                .insetBy(dx: -strokeOutset, dy: -strokeOutset)
        case .rectangle, .ellipse, .mosaicRectangle, .text, .numberSequence, .magnifier:
            return annotation.rect.standardized
        }
    }

    private func localInteractionBounds(for mask: CaptureEraserMask) -> NSRect? {
        switch mask.kind {
        case .rectangle:
            return mask.rect.standardized
        case .freehand:
            guard !mask.points.isEmpty else {
                return nil
            }
            let radius = max(1, mask.size / 2)
            return localBounds(of: mask.points).insetBy(dx: -radius, dy: -radius)
        }
    }

    private func localBounds(of points: [NSPoint]) -> NSRect {
    ...
    private func brushRotationHitTarget(at point: NSPoint) -> (index: Int, target: SelectionToolbarState.BrushRotationHitTarget)? {
        guard let selectedAnnotationIndex,
              annotations.indices.contains(selectedAnnotationIndex),
              !isAnnotationErasedForInteraction(at: selectedAnnotationIndex),
