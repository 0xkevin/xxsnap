...
为保证 18×18 点状态栏尺寸下的可读性，对生成的 128px alpha 遮罩做 3px 均匀内缩。三条内部透明接缝会同步加宽，在 Retina 约 36px 的实际像素尺寸下从接近 1px 增加到约 2px；外轮廓每侧缩小不到 1 个显示像素，不改变折纸造型。
...
- 菜单栏源图超过一半像素完全透明，同时至少十分之一像素保持可见，三条内部接缝在 36px 预览下均清晰可辨，仍按模板图显示。
- Xcode Debug 构建成功。
