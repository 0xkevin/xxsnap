...
The Pin toolbar button should create a floating, always-on-top image window from the current locked selection instead of showing a placeholder.
...
The first version supports moving, scaling, and closing a pinned window while deferring opacity, click-through, layer presets, and multi-window management UI.
...
- R1. Clicking Pin on a locked selection creates a pinned image window and closes the capture overlay instead of showing `贴图功能开发中。`.
...
- R4. A pinned image window stays above normal app windows, can be dragged to move, can be scaled without changing aspect ratio, and can be closed.
...
- AE1. Given a locked selection with no annotations, when the user clicks Pin, then the overlay closes and a floating window appears with the selected image.
...
- AE5. Given a pinned image window, when the user clicks the close control or presses Escape while it is focused, then the pinned window closes without ending the app.
...
- A2. Scaling can use a testable AppKit-native gesture path and a visible close affordance; richer handles and menus are follow-up UX polish.
...
flowchart TB
  ...
  J --> K["Move / scale / close"]
```

### Risks & Dependencies

- The toolbar button currently shares placeholder routing with scroll capture; the change must target Pin only.
- AppKit borderless floating windows can become hard to close if the window lacks a visible close path, so the first version needs both keyboard and pointer close affordances.
...
- **Goal:** Add a mac pinned-window implementation that owns image display, initial fitting, move, scale, and close behavior.
- **Requirements:** R4, R5.
- **Dependencies:** None.
...
- **Approach:** Add a dedicated AppKit controller/view pair for a borderless floating window. The image view draws the rendered capture, body drag moves the window, supported scale input resizes around the current frame while preserving aspect ratio, Escape and a visible close control close the window. Extract size clamping into a pure helper so tests can cover min/max and aspect behavior.
...
- **Test scenarios:** Initial window size fits within a percentage of visible screen while preserving aspect; scaling up and down preserves aspect and clamps to min/max; close action marks the controller/window closed; drag calculation preserves the click offset.
...
- **Approach:** After the existing crop/capture and annotation render step, switch `.pin` to create and retain a `PinnedImageWindowController`. Do not write to pasteboard or invoke save panel. Keep `lastCapture` updated with the rendered image so DEBUG tests and future actions see the final pixels.
- **Patterns to follow:** Existing copy/save coordinator flow and legacy Qt controller tests for pin compositing.
- **Test scenarios:** Pin from a frozen image opens one pinned controller with the cropped image size; Pin with an annotation uses the renderer output; Pin with eraser masks passes masks into the renderer; Pin does not modify the pasteboard; empty or nil results do not create a pinned window.
...
- **Approach:** Keep the existing Pin icon and tooltip, add any missing DEBUG helper mapping needed for tests, update requirements status from unfinished to implemented, and add a concise user-guide section for move, scale, and close controls.
- **Patterns to follow:** Recent docs updates for magnifier and eraser, and existing toolbar tooltip tests.
- **Test scenarios:** Pin tooltip remains `贴图`; Pin icon remains available; unsupported tool placeholder tests cover Scroll only where relevant; docs mention Pin as supported and no longer describe it as unfinished.
...
| Manual smoke | U1-U4 | Launch the debug app from `build/xcode-derived`, capture a region, click Pin, move/scale/close the pinned window. | Pinned window appears above normal windows and can be moved, scaled, and closed. |
...
- Users can move, scale, and close each pinned image window.
